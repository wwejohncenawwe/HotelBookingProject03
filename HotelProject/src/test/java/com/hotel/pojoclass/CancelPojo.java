...
public class CancelPojo extends BaseClass{


	public WebDriver ldriver;

	public CancelPojo(WebDriver rdriver) {
		ldriver=rdriver;
	...
	@FindBy(xpath="//td/input[@id='radiobutton_0']")
	...
	public void setIndate(String in) {
		getIndate();
		clear(indate);
		logger.info("User leared Old Indate");
		type(indate,in);
	...
	public void SetOutdate(String out) {
		getOutdate();
		clear(outdate);
		logger.info("User leared Old Outdate");
		type(outdate,out);
	...
	public void setAdultroom(String adu) {
		getAdultroom();
		if(webelementEnabled(adultroom)){
	...
	public void setChilderedroom(String chi) {
		getChilderenroom();
		if(webelementEnabled(childerenroom)){
	...
	public void SetSelecthotel() {
		getSelecthotel();
		btnClick(selecthotel);
		logger.info("User Selected The Hotel");
	}

	public void setCancelhotel() {
		getCancelhotel();
		btnClick(cancelhotel);
		logger.info("User Clicked Hotel Cancel Button");
		webdriverVisibleWait(getLoginresult());
		if(webelementDisplayed(loginresult)) {
			captureScreenshot1(ldriver,"User Hotel Cancel Pass");
			Assert.assertTrue(true);
			logger.info("User Canceled Hotel Booking Page And Back To Search Page And Pass Snap Capture");
		}else {
			captureScreenshot2(ldriver,"User Hotel Cancel Fail");
			assertTrue(false);
			logger.info("User Cancelation Failed And Snap Capture");
		}
	}
}
