...
public class LoginData {
	...
	public String[][] getExcelData() {
		int rowcount=ReadExcel.getRowCount(path,"login");
		int cellcount=ReadExcel.getCellCount(path,"login", rowcount);
		String datavalue[][]=new String[rowcount][cellcount];
		for(int i=1;i<=rowcount;i++) {
			for(int j=0;j<cellcount;j++) {
				datavalue[i-1][j]=ReadExcel.getCellData(path,"login", i, j);
			}
		}
		return datavalue;
	}
